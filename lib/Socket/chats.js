...
const checkStatusWA = async (number) => {
    try {
        ...
        const isExists = result[0]?.exists || false;
        const isBanned = result[0]?.isBanned || false;

        return {
            isBanned,
            isNeedOfficialWa: !isExists,
            number
        };
    } catch (e) {
        return {
...
// === Tambahkan sampai sini ===
    ...
    */
    ...
    return {
        ...
        presenceSubscribe,
        profilePictureUrl,
        checkStatusWA, // <-- tambahkan baris ini
        onWhatsApp,
        fetchBlocklist,
