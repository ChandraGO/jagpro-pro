...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, keyStore, logger, options, getMessage }) => {
    ...
    const content = (0, messages_1.normalizeMessageContent)(message.message);
    // ✅ Tambahan dukungan groupStatusMessage
if (normalizedContent.groupStatusMessage) {
    const status = normalizedContent.groupStatusMessage
    const text = status.text || ''

    return {
        messageStubType: 'GROUP_STATUS_MESSAGE',
        messageType: 'groupStatusMessage',
        text,
        key: msg.key,
        isGroupStatus: true,
    }
