...
async function getAudioWaveform(buffer, logger) {
    try {
        ...
        const audioData = await exports.toBuffer(buffer);
        const audioBuffer = await decoder(audioData);
        const rawData = audioBuffer.getChannelData(0);

        // Normalisasi data gelombang
        const normalizeArray = (data, samples = 64) => {
            const blockSize = Math.floor(data.length / samples);
            const filteredData = [];
            for (let i = 0; i < samples; i++) {
                let blockStart = blockSize * i;
                let sum = 0;
                for (let j = 0; j < blockSize; j++) sum += Math.abs(data[blockStart + j]);
        ...
        const normalizedData = normalizeArray(rawData);
